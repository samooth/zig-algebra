...
pub fn main() !void {
    std.debug.print("=== zig-merkle example ===\n\n", .{});
    ...
    // --- Classic Merkle Tree ---
    std.debug.print("--- Classic Merkle Tree ---\n", .{});
    ...
    printHex("Merkle root", &root);

    // Prove inclusion for tx3
    const proof = try tree.prove(2, allocator);
    defer proof.deinit(allocator);
    std.debug.print("Proof for tx3 has {} siblings\n", .{proof.siblings.len});
    ...
    std.debug.print("Serialized proof size: {} bytes\n", .{serialized.len});
    ...
    std.debug.print("SMT verify account A: {}\n", .{smt_ok});
    ...
    std.debug.print("SMT non-membership for 50: {}\n", .{non_ok});
